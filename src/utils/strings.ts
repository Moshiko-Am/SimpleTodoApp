export const stringToArray: (str: string | string[] | undefined | null) => string[] = (str) => {
...
export const generateModifierArray: (modifier?: TModifiers) => string[] = (modifier) => {
	...
	if (Array.isArray(modifier)) {
		return modifier.reduce((acc: string[], curr: string | IModifier) => {
			...
			if (typeof curr === 'object' && curr.name) {
				return curr?.condition ? [...acc, ...(curr?.name?.trim() ? [String(curr?.name)] : [])] : [...acc];
			}
			return [...acc];
		}, []);
	}
...
export const generateBEMClassName: (params: IGenerateBEMBlockParams) => string = ({ block, element, modifier, utilities }) => {
	const blockArray: string[] = stringToArray(block);
	const elementArray: string[] = stringToArray(element);
	const modifierArray: string[] = generateModifierArray(modifier).filter(Boolean);
	const utilityArray: string[] = stringToArray(utilities);
		...
		if (elementArray?.length <= 0) {
			...
			elementArray?.forEach((element) => {
				if (element && element?.trim()) {
					...
					modifierArray?.forEach((modifier) => {
						if (modifier && modifier?.trim()) {
							className += `${block}${element ? `__${element}` : ''}${modifier ? `--${modifier}` : ''} `;
						}
					});
				}
			});
